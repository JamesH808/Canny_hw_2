...
void make_gaussian_kernel(float sigma, float kernel[ROWS*COLS], int *windowsize);
...
void apply_hysteresis(short int *mag, unsigned char nms[ROWS*COLS], int rows, int cols,
...
{
   const char *infilename = INFILENAME;  /* Name of the input image */
   ...
   float sigma = SIGMA;              /* Standard deviation of the gaussian kernel. */
   float tlow = TLOW;               /* Fraction of the high threshold in hysteresis. */
   float thigh = THIGH;              /* High hysteresis threshold control. The actual
         ...
         float tlow, float thigh, unsigned char *edge, char *fname)
{
   ...
   short int smoothedim[ROWS*COLS],     /* The image after gaussian smoothing.      */
             delta_x[ROWS*COLS],        /* The first devivative image, x-direction. */
             delta_y[ROWS*COLS],        /* The first derivative image, y-direction. */
             magnitude[ROWS*COLS];      /* The magnitude of the gadient image.      */
  /* int r, c, pos;*/
        ...
        short int smoothedim[ROWS*COLS])
{
   int r, c, rr, cc,     /* Counter variables. */
      windowsize = 21,        /* Dimension of the gaussian kernel. */
      center;            /* Half of the windowsize. */
   float tempim[ROWS*COLS],        /* Buffer for separable filter gaussian smoothing. */
         kernel[ROWS*COLS],        /* A one dimensional gaussian kernel. */
